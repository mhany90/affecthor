...
def cross_validate(feat_file, dev_feat_file, test_feat_file, tok_file, dev_tok_file, test_tok_file,  n_folds, CV):
    print(feat_file)
    ...
    if CV:
        ...
        top_systems = ['preds_dnn', 'preds_gbt', 'preds_char_lstm']
    ...
    run_times = 5
    all_dnn, all_cnn_lstm, all_char_lstm, all_gbt, all_cnn = [],[],[],[],[]
    ...
    #preds_cnn = np.mean(all_cnn, axis = 0)
   ...
   # all = {'preds_dnn':preds_dnn, 'preds_cnn_lstm':preds_cnn_lstm, 'preds_gbt':preds_gbt, 'preds_cnn':preds_cnn, 'preds_char_lstm':preds_char_lstm}
    all = {'preds_dnn':preds_dnn, 'preds_gbt':preds_gbt, 'preds_char_lstm':preds_char_lstm}
...
cross_validate(feat_file, dev_feat_file, test_feat_file, tok_file, dev_tok_file, test_tok_file, 2, CV)
